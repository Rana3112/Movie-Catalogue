...
void main () {
  ...
  float fy = uv.y / zone;
  if (fy > 1.0) { outColor = vec4(0.0); return; }
...
void main () {
  ...
  const firePass = linkShader(FIRE_FRAG);
  const mainPass = linkShader(FRAG);

  function linkShader(fragText) {
...
export default function Blaze({
  ...
  useEffect(() => {
    ...
    instanceRef.current = createBlaze(
      ...
      { height, distortion, distortionScale, speed, sparks, sparkDensity, sparkSize, layers, smoke, glow, sparkColor, smokeColor },
    );
  ...
  useEffect(() => {
    instanceRef.current?.setOptions({ height, distortion, distortionScale, speed, sparks, sparkDensity, sparkSize, layers, smoke, glow, sparkColor, smokeColor });
  });
