...
const getUnauthorizedDomainMessage = () => {
  ...
  if (hostname === '127.0.0.1') {
    return 'Google signup is blocked for 127.0.0.1. Open http://localhost:5173 instead, or add 127.0.0.1 in Firebase Auth > Settings > Authorized domains.'
  }
  ...
  return `Google signup is blocked for ${hostname}. Add ${hostname} and ${pairedDomain} in Firebase Auth > Settings > Authorized domains.`
}
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          lastError = fetchErr
          console.error(`[Signup] Fetch attempt ${attempt + 1} failed:`, fetchErr.message)
          if (attempt < 2) {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Google Signup failed on server')
      }
    } catch (err) {
      ...
      >
        ...
        <div className="mt-8 text-center">
          <p className="text-white/40 text-sm">
            Already have an account?{' '}
            <Link to="/login" className="text-[#ffd700] active:underline">
              Sign In
            </Link>
          </p>
        </div>
